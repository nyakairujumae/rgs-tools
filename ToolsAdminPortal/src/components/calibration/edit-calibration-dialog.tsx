...
import { updateCalibrationRecord } from '@/lib/supabase/actions'
...
export function EditCalibrationDialog({ certification, open, onClose, onSuccess }: EditCalibrationDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const updated = await updateCalibrationRecord(certification.id, {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-card border border-border rounded-xl w-full max-w-[520px] mx-4 shadow-xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-4 border-b border-border">
          <div>
            <h2 className="text-lg font-semibold">Edit Calibration</h2>
