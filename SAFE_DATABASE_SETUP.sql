...
-- Safe database setup that handles missing tables gracefully

-- 1. Drop existing tables in correct order (only if they exist)
DROP TABLE IF EXISTS public.tools CASCADE;
DROP TABLE IF EXISTS public.users CASCADE;
DROP TABLE IF EXISTS public.technicians CASCADE;

-- 2. Drop existing policies (only if they exist)
...
-- 3. Drop existing triggers and functions (only if they exist)
