...
-- Insert some sample data for testing (only if tools exist)
DO $$
BEGIN
  -- Only insert sample data if there are tools in the tools table
  IF EXISTS (SELECT 1 FROM tools LIMIT 1) THEN
    INSERT INTO tool_issues (tool_id, tool_name, reported_by, issue_type, description, priority, status, location) VALUES
    (
      (SELECT id FROM tools LIMIT 1),
      'Sample Tool',
      'John Doe',
      'Faulty',
      'Tool is not working properly, makes strange noises when operating',
      'High',
      'Open',
      'Job Site A'
    ),
    (
      (SELECT id FROM tools LIMIT 1 OFFSET 1),
      'Another Tool',
      'Jane Smith',
      'Lost',
      'Tool was left at job site and cannot be located',
      'Critical',
      'Open',
      'Job Site B'
    ),
    (
      (SELECT id FROM tools LIMIT 1 OFFSET 2),
      'Third Tool',
      'Mike Johnson',
      'Damaged',
      'Tool was dropped and has visible damage to the housing',
      'Medium',
      'In Progress',
      'Warehouse'
    );
  ELSE
    -- If no tools exist, just create the table structure
    RAISE NOTICE 'No tools found in tools table. Skipping sample data insertion.';
  END IF;
END $$;
