      ...
      for (final file in archive.files) {
        if (file.isFile) {
          ...
          if (file.name.startsWith('xl/worksheets/sheet') && file.name.endsWith('.xml')) {
            ...
            // Normalized page setup / margins snippets we want to enforce
            const pageSetupTag = '<pageSetup orientation="landscape" fitToWidth="1" fitToHeight="0" horizontalDpi="300" verticalDpi="300" scale="100"/>';
            const pageMarginsTag = '<pageMargins left="0.35" right="0.35" top="0.5" bottom="0.5" header="0.3" footer="0.3"/>';
            
            // Ensure a sheet view zoom is present so content fills the screen
            xmlContent = xmlContent.replaceFirstMapped(
              RegExp(r'<sheetView([^>]*)/>'),
              (match) {
                final attrs = match.group(1) ?? '';
                if (attrs.contains('zoomScale')) {
                  return match.group(0)!;
                }
                return '<sheetView${attrs} zoomScale="140" zoomScaleNormal="100" zoomScaleSheetLayoutView="140"/>';
              },
            );
            xmlContent = xmlContent.replaceFirstMapped(
              RegExp(r'<sheetView([^>]*)>'),
              (match) {
                final attrs = match.group(1) ?? '';
                if (match.group(0)!.contains('zoomScale')) {
                  return match.group(0)!;
                }
                return '<sheetView${attrs} zoomScale="140" zoomScaleNormal="100" zoomScaleSheetLayoutView="140">';
              },
            ...
            // Update or insert pageMargins
            if (xmlContent.contains('<pageMargins')) {
            ...
            // Update or insert pageSetup to force landscape + fit-to-width
            if (xmlContent.contains('<pageSetup')) {
              ...
              xmlContent = xmlContent.replaceAll(
                RegExp(r'<pageSetup[^>]*>.*?</pageSetup>', dotAll: true),
                pageSetupTag,
              );
            } else {
              final worksheetEnd = xmlContent.indexOf('</worksheet>');
