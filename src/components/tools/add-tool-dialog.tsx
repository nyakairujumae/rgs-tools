...
import { X, Loader2, Camera, RefreshCw, Plus, Trash2 } from 'lucide-react'
...
const CATEGORIES = [
  ...
  'Plumbing Tools', 'Carpentry Tools', 'Automotive Tools', 'Garden Tools',
  'HVAC Tools', 'Cleaning Equipment', 'Other',
]

const CONDITIONS = ['Excellent', 'Good', 'Fair', 'Poor']

// Matches Flutter app's ToolIdGenerator
...
function generateSerialNumber() {
  ...
  return `RGS-${year}-${randomHex(6)}`
}

function generateModelNumber() {
  return `MDL-${randomHex(4)}`
}
...
export function AddToolDialog({ open, onClose, onSuccess }: AddToolDialogProps) {
  ...
  const handleGenerateSerial = () => {
    updateField('serial_number', generateSerialNumber())
  }

  const handleGenerateModel = () => {
    updateField('model', generateModelNumber())
  }
