...
if [ -f "$GENERATED_FILE" ]; then
    ...
    # Don't comment out OpenFilePlugin - it's needed for opening PDFs
    # Use dynamic registration instead
    perl -i -0pe 's/^\s*\[OpenFilePlugin registerWithRegistrar:\[registry registrarForPlugin:@"OpenFilePlugin"\]\];\s*$/  Class OpenFilePluginClass = NSClassFromString(@"OpenFilePlugin");\n  if (OpenFilePluginClass) {\n    [OpenFilePluginClass performSelector:@selector(registerWithRegistrar:) withObject:[registry registrarForPlugin:@"OpenFilePlugin"]];\n  }\n/gm' "$GENERATED_FILE"
