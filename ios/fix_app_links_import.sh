...
if [ -f "$GENERATED_FILE" ]; then
    ...
    sed -i '' 's/^  \/\/ \[OpenFilePlugin registerWithRegistrar:\[registry registrarForPlugin:@"OpenFilePlugin"\]\]; \/\/ Swift-only$/  Class OpenFilePluginClass = NSClassFromString(@"OpenFilePlugin");\n  if (OpenFilePluginClass) {\n    [OpenFilePluginClass performSelector:@selector(registerWithRegistrar:) withObject:[registry registrarForPlugin:@"OpenFilePlugin"]];\n  }/g' "$GENERATED_FILE"
